...
const TechBackground = ({ theme }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(20,184,166,${theme === 'light' ? '0.02' : '0.05'})_0%,transparent_70%)]`}></div>
  ...
  style={{
    ...
    left: `${config.left}%`,
    top: `${config.top}%`,
    background: `radial-gradient(circle, rgba(20, 184, 166, ${theme === 'light' ? '0.05' : '0.1'}) 0%, rgba(0,0,0,0) 70%)`
  }}
/>
      ))}
  ...
  key={`tech-${idx}`}
  animate={{ 
    opacity: theme === 'light' ? [0.05, 0.1, 0.05] : [0.1, 0.3, 0.1],
    x: [0, tech.xRand, 0],
    y: [0, tech.yRand, 0],
  }}
  transition={{ 
  ...
  className={`absolute font-black text-[10px] md:text-[12px] tracking-[0.5em] uppercase whitespace-nowrap will-change-transform ${theme === 'light' ? 'text-primary/10' : 'text-primary/20'}`}
